...
import numpy as np
import os, logging, csv, collections, librosa, config, datetime, pickle, audioread
...
class TargetProcessor(object):
    ...
    def process(self, start_time, midi_events_time, midi_events, extend_pedal=True, note_shift=0):
        ...
        midi_events_time : list[float]
            times of MIDI events of a recording, e.g. [0, 3.3, 5.1, ...]
        midi_events : list[str]
            MIDI events of a recording,\n
            e.g.
            ['note_on channel=0 note=75 velocity=37 time=14',
             'control_change channel=0 control=64 value=54 time=20',
             ...]
        extend_pedal : bool, optional
            True: Notes will be set to ON until pedal is released. False: Ignore pedal events,\n
            by default True
        note_shift : int, optional
            by default 0
    ...
    def get_regression(self, input):
        ...
        -------
        (frames_num,)
            e.g., [0, 0, 0.1, 0.3, 0.5, 0.7, 0.9, 0.9, 0.7, 0.5, 0.3, 0.1, 0, 0, ...]
        """
...
def plot_waveform_midi_targets(data_dict, start_time, note_events):
    """For debugging. Write out waveform, MIDI and plot targets for an audio segment.
...
class RegressionPostProcessor(object):
    def __init__(self, frames_per_second, classes_num, onset_threshold, offset_threshold, frame_threshold, pedal_offset_threshold):
        """Postprocess the output probabilities of a transription model to MIDI events.
    ...
    def get_binarized_output_from_regression(self, reg_output, threshold, neighbour):
        """Calculate binarized output and shifts of onsets or offsets from the regression results.
    ...
    def output_dict_to_detected_notes(self, output_dict):
        ...
        -------
        _type_
            est_on_off_note_vels: (notes, 4), the four columns are onsets, offsets, MIDI notes and velocities. 
            E.g.,
    ...
    def detected_notes_to_events(self, est_on_off_note_vels):
        ...
        Parameters
        ----------
        est_on_off_note_vels : (notes, 3)
            the three columns are onset_times,
    ...
    def detected_pedals_to_events(self, pedal_on_offs):
        ...
        Parameters
        ----------
        pedal_on_offs : (notes, 2)
            the two columns are pedal onsets and pedal offsets.
            E.g.,
            [[0.1800, 0.9669],
             [1.1400, 2.6458],
             ...]
...
class OnsetsFramesPostProcessor(object):
    def __init__(self, frames_per_second, classes_num):
        """Postprocess the Googl's onsets and frames system output. Only used for comparison.
    ...
    def sharp_output_dict(self, output_dict, onset_threshold, offset_threshold):
        """Sharp onsets and offsets\n
        E.g. when threshold=0.3, for a note,
    ...
    def sharp_output(self, input, threshold=0.3):
        """Used for sharping onset or offset.\n
        E.g. when threshold=0.3, for a note,
        [0, 0.1, 0.4, 0.7, 0, 0] will be sharped to [0, 0, 0, 1, 0, 0]
